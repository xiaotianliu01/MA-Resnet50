...
class DeepLabV3P(nn.Layer):
    ...
    def forward(self, x, clr):
        ...
        if(clr == False):
            for i in range(256):
                a = feat_list[0].numpy()*255
                a = a[0][i][:][:]
                cv2.imwrite('feat/'+str(i)+'.png', a)
        '''
...
class DeepLabV3PHead(nn.Layer):
    ...
    def forward(self, feat_list, op_attention=None):
        logit_list = []
        low_level_feat = feat_list[self.backbone_indices[0]]
        x = feat_list[self.backbone_indices[1]]
        x = self.aspp(x)
        logit = self.decoder(x, low_level_feat, op_attention)
...
class Decoder(nn.Layer):
    ...
    def forward(self, x, low_level_feat, op_attention = None):
