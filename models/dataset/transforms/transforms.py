...
class Compose:
    ...
    def __init__(self, transforms, to_rgb=True):
        ...
        for op in transforms:
            if(op['type'] == 'ResizeStepScaling'):
                tem = ResizeStepScaling(min_scale_factor = op['min_scale_factor'], 
                                max_scale_factor = op['max_scale_factor'],
                                scale_step_size = op['scale_step_size'])
                self.transforms.append(tem)
            elif(op['type'] == 'RandomPaddingCrop'):
                tem = RandomPaddingCrop(crop_size=[960, 720])
                self.transforms.append(tem)
            elif(op['type'] == 'RandomHorizontalFlip'):
                tem = RandomHorizontalFlip()
                self.transforms.append(tem)
            elif(op['type'] == 'RandomDistort'):
                tem = RandomDistort(brightness_range=0.4,
                                    contrast_range=0.4,
...
class Resize:
    ...
    def __init__(self, target_size=(640, 640), interp='NEAREST'):
        self.interp = interp
